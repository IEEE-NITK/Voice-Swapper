...
class CycleGAN(object):
    ...
    def forward(self, input_A_real, input_B_real, lambda_cycle, lambda_identity, generator_learning_rate, discriminator_learning_rate,gen_tape,disc_tape):

        # Placeholders for real training samples
        # self.input_A_real = tf.compat.v1.placeholder(tf.float32,shape = self.input_shape, name = 'input_A_real')
        # self.input_B_real = tf.compat.v1.placeholder(tf.float32,shape = self.input_shape, name = 'input_B_real')
        # # Placeholders for fake generated samples
        # self.input_A_fake = tf.compat.v1.placeholder(tf.float32, shape = self.input_shape, name = 'input_A_fake')
        # self.input_B_fake = tf.compat.v1.placeholder(tf.float32, shape = self.input_shape, name = 'input_B_fake')
        # # Placeholder for test samples
        # self.input_A_test = tf.compat.v1.placeholder(tf.float32, shape = self.input_shape, name = 'input_A_test')
        # self.input_B_test = tf.compat.v1.placeholder(tf.float32, shape = self.input_shape, name = 'input_B_test')

        self.generatorA2B_variables,self.generation_B = self.generator(inputs = input_A_real, reuse = False, scope_name = 'generator_A2B')
        self.generatorB2A_variables,self.cycle_A = self.generator(inputs = self.generation_B, reuse = False, scope_name = 'generator_B2A')

        self.generation_A = self.generator(inputs = input_B_real, reuse = True, scope_name = 'generator_B2A')
        self.cycle_B = self.generator(inputs = self.generation_A, reuse = True, scope_name = 'generator_A2B')

        self.generation_A_identity = self.generator(inputs = input_A_real, reuse = True, scope_name = 'generator_B2A')
        self.generation_B_identity = self.generator(inputs = input_B_real, reuse = True, scope_name = 'generator_A2B')

        self.discriminatorA_variables,self.discrimination_A_fake = self.discriminator(inputs = self.generation_A, reuse = False, scope_name = 'discriminator_A')
        self.discriminatorB_variables,self.discrimination_B_fake = self.discriminator(inputs = self.generation_B, reuse = False, scope_name = 'discriminator_B')
        ...
        # self.lambda_cycle = tf.compat.v1.placeholder(tf.float32, shape=None, name = 'lambda_cycle')
        self.lambda_cycle = lambda_cycle
        # self.lambda_identity = tf.compat.v1.placeholder(tf.float32, shape=None, name = 'lambda_identity')
        ...
        # Compute gradients
        self.generator_gradients = gen_tape.gradient(self.generator_loss, [self.generatorA2B_variables,self.generatorB2A_variables])
        self.discriminator_gradients = disc_tape.gradient(self.disc_loss, [self.discriminatorA_variables,self.discriminatorB_variables])
         
        # Apply gradients
        self.generator_optimizer.apply_gradients(zip(self.generator_gradients, [self.generatorA2B_variables,self.generatorB2A_variables]))
        self.discriminator_optimizer.apply_gradients(zip(self.discriminator_gradients, [self.discriminatorA_variables,self.discriminatorB_variables]))

        self.writer.add_summary(tf.summary.scalar("Gen Loss: ",self.generator_loss, step = self.train_step)).numpy()
        self.writer.add_summary(tf.summary.scalar("Disc Loss: ",self.discriminator_loss, step = self.train_step)).numpy()
        self.train_step += 1
        
        return self.generator_loss, self.discriminator_loss

    def optimizer_initializer(self,default_d_rate = 0.0001,default_g_rate=0.0002):
        
        self.generator_learning_rate = tf.compat.v1.placeholder(tf.float32, shape=None, name = 'generator_learning_rate')
        self.discriminator_learning_rate = tf.compat.v1.placeholder(tf.float32, shape=None, name = 'discriminator_learning_rate')
        self.discriminator_optimizer = tf.keras.optimizers.Adam(learning_rate =default_d_rate, beta_1 = 0.5) 
        #.minimize(self.discriminator_loss, var_list = self.discriminator_vars)
        self.generator_optimizer = tf.keras.optimizers.Adam(learning_rate = default_g_rate, beta_1 = 0.5)
